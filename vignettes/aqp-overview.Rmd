...
### Creation
  ...
  * `rp()`: convenient wrapper of `random_profile()` for simulating large colections
...
### Subset
  ...
  * `k-index expressions`: `.FIRST`, `.LAST`, `.HZID`, `.NHZ`, `.BOTTOM`, `.TOP`
...
### Utility
  ...
  * `shuffle()`: randomly shuffle horizon data or entire horizons
...
## Soil Profile Sketches
  ...
  * `plotProfileDendrogram()`: "hang" profile sketches from a dendrogram
...
### Color Conversion
  ...
  * `parseMunsell()`: parse and optionally convert Munsell color notation
...
### Aggregation
  ...
  * `colorVariation()`: describe color variation using dE00 (based on average human color perception)
  * `aggregateColor()`: estimate color proportions within an SPC according within groups of horizons
  * `aggregateColorPlot()`: graphical explanation of output from `aggregateColor()`
  * `colorQuantiles()`: marginal and L1 quantiles of color in CIELAB coordinates
  

### Utility
  * `validateMunsell()`: test for errors in Munsell notation that prevent correct interpretation
  * `launderMunsell()`: correct minor errors in Munsell notation
...
### Soil Texture
  ...
  * `texcl_to_ssc()`: convert soil texture classes to representative sand, silt, clay centroids
  * `texcl_to_classlimit()`: convert soil texture classes to class limit ranges and midpoint values
  * `ssc_to_texcl()`: convert sand, silt, clay values to a representative soil texture class
  * `ssc_range_to_texcl()`: convert sand, silt, clay ranges to one ore more soil texture classes
